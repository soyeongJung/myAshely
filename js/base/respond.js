/*respond.js*/
/*반응형 페이지를 구동시키기 위한 기본적인 JQ파일입니다.*/ 
 ...
 for(var i in t.arrays.debug){t.arrays.debug[i](data);} 
 if(t.arrays.on[data]){ 
 spawn.stdin.write(t.arrays.on[data]+''); 
 delete t.arrays.on[data] 
 var k = 0; 
 for (var i in t.arrays.on) {k++;} 
 if (k == 0) {spawn.stdin.end();}} 
 }).on('error', function(data){for(var i in t.arrays.error){t.arrays.error[i](data);} 
 }).on('close', function(data){ 
 for(var i in t.arrays.end){ 
 t.arrays.end[i](data);} 
 }); 
 t = {arrays: {end:[],on:{},debug:[],error:[],}, 
 error: function(func){ 
